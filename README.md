...
그러면 AI 기획자가 `new-setup.md`를 읽고 아래 내용을 질문하며 초기 설정을 진행합니다:
- 기존 프로젝트 컨텍스트 파일이 있는지 (없으면 `CLAUDE.md` 자동 생성)
- 기본 기술 스택 변경이 필요한지
- 역할별 모델을 따로 지정할지 (기본: 현재 세션 모델)

설정이 완료되면 AI가 "이 파일은 더 이상 필요하지 않습니다"라고 알려줍니다.

### 5. 이후 매 세션 시작

설정 완료 후 매 세션 시작 시, **하고자 하는 일에 맞는 역할 프롬프트를 직접 붙여넣습니다.**

| 역할 | 언제 사용 |
|------|----------|
| 기획자 | 요구사항 정의, 설계, 작업 지시, 배포 — **일반적으로 여기서 시작** |
| 개발자 | 구현에 바로 집중하고 싶을 때 |
| 검토자 | 코드 리뷰·품질 점검만 할 때 |

**▼ Claude Code 입력창에 아래 중 하나를 붙여넣으세요**
...
```
# 개발자로 시작
이 프로젝트의 개발자 역할입니다. .claude/CLAUDE.md를 읽은 후 BUILDER.md를 읽어주세요.
```

```
# 검토자로 시작
이 프로젝트의 검토자 역할입니다. .claude/CLAUDE.md를 읽은 후 REVIEWER.md를 읽어주세요.
```

> 기획자가 구현·검토가 필요하면 직접 서브에이전트(개발자·검토자)를 실행합니다.
> 사용자는 보통 기획자 프롬프트만 사용하면 됩니다.
